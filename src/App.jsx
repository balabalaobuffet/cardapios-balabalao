...
import { ChevronRight, ChevronLeft, Check, Sparkles, PartyPopper, Crown, Gift, Coffee, CakeSlice, Utensils, GlassWater, IceCreamCone, ChefHat, Sandwich, Palette, Send, MessageCircle, Mail, CircleCheck, Circle, Info, Clock, Star, ArrowRight, X, Phone, User, CalendarDays, Baby, Hash } from "lucide-react";
...
const ICON_MAP = {
  utensils: Utensils, glass: GlassWater, candy: Star, cake: CakeSlice,
  gift: Gift, coffee: Coffee, icecream: IceCreamCone, chef: ChefHat,
                            ...
                            <div style={{ fontSize: 13, color: T.text, display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center" }}>
                              {s.items.map((it, idx) => (
                                <span key={it.id} style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
                                  <CircleCheck size={11} color={T.green} style={{ flexShrink: 0 }} />
                                  <span>{it.name}</span>
...
const CategorySelectPage = ({ onNavigate, onSelect }) => {
  const catIcons = { promocionais: Sparkles, tradicionais: PartyPopper, premium: Crown };
  return (
    <Page>
  ...
  return (
    <Page>
      ...
      <div className="anim-fade-up">
        ...
        <Field label="Data da festa" icon={CalendarDays} value={info.date} type="date" min={today}
...
const Counter = ({ current, max, label }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px",
      ...
      {done && <CircleCheck size={14} color={T.green} />}
    </div>
  );
};
  ...
  <Page>
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
      ...
      <div className="anim-scale-in" style={{ width: 80, height: 80, borderRadius: "50%", background: T.greenBg,
        display: "flex", alignItems: "center", justifyContent: "center" }}>
        <CircleCheck size={40} color={T.green} />
      </div>
